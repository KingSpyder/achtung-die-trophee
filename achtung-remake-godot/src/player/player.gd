...
## Set the player's collision layer and mask to collide with everything except its own RecentTrail layer.
...
## Check whether this playerScene belongs to the client. CURRENTLY BYPASSED, as we don't have multiplayer implemented, but will be useful when we do.
...
## Identify the type of collider and print a message. Return true if the collision should cause death, false if it should be ignored (e.g. touching own recent trail).
...
## Check if the player is out of bounds (i.e. outside the 800x800 area). Return true if out of bounds, false otherwise.
...
## Player death function. Stop the player's movement and emit the player_died signal to notify the game logic controller.
func death() -> void:
