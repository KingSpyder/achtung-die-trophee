...
func init_players() -> void:
	players_selection_nodes = [
		...
		PLAYER_SELECTION_SCENE.instantiate().init_data(
			"3", PlayersConstants.PINKNEY_NAME, PlayersConstants.PINKNEY_COLOR
		),
		PLAYER_SELECTION_SCENE.instantiate().init_data(
			"2", PlayersConstants.GREENLEE_NAME, PlayersConstants.GREENLEE_COLOR
		),
		PLAYER_SELECTION_SCENE.instantiate().init_data(
			"1", PlayersConstants.FRED_NAME, PlayersConstants.FRED_COLOR
		),
	]
...
func _on_arcade_button_toggled(toggled_on: bool) -> void:
	if toggled_on:
		GameManager.game_mode = GameManager.GameMode.ARCADE_MODE


func _on_classic_button_toggled(toggled_on: bool) -> void:
	if toggled_on:
		GameManager.game_mode = GameManager.GameMode.CLASSIC_MODE
