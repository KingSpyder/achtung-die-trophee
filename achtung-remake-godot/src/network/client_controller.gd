...
func create_match() -> void:
	...
	print(
		(
			"New match with id %s from %s with %s people in"
			% [signaling_match.match_id, match_code, signaling_match.presences.size()]
		)
	)
...
#	# The host will always have 1 as a peer_id
	...
	print(
		(
			"joined with id %s from %s with %s people in"
			% [signaling_match.match_id, match_code, signaling_match.presences.size()]
		)
	)
...
func create_peer(peer_id: int) -> void:
	...
	peer.initialize({"IceServers": NetworkConfigs.ICE_SERVERS})
...
func _offer_created(type: String, data: String, peer_id: int) -> void:
	...
	if type == "offer":
		ServerController.send_offer(signaling_match.match_id, data, rtc_mp.get_unique_id())
	else:
		ServerController.send_answer(signaling_match.match_id, data, peer_id)
...
func _on_send_message_button_down() -> void:
	ServerController.send_message(
		signaling_match.match_id, rtc_mp.get_unique_id(), "Salut à tous !!"
	)
