   ...
   public class ComposeState extends BaseCompose
   {
      ...
      tlf_internal static function releaseComposeState(param1:ComposeState) : void
      {
         if(_sharedComposeState == null)
         {
            ...
            {
               _sharedComposeState.tlf_internal::releaseAnyReferences();
            }
         }
      }
      ...
      override protected function initializeForComposer(param1:IFlowComposer, param2:int, param3:int) : void
      {
         ...
         var _loc4_:int = param1.findControllerIndexAtPosition(param1.damageAbsoluteStart);
         if(_loc4_ == -1)
         {
      ...
      override protected function composeInternal(param1:FlowGroupElement, param2:int) : void
      {
         ...
         if(_curElement)
         {
            ...
            {
               _flowComposer.getLineAt(this._curLineIndex++).tlf_internal::setController(null,-1);
            }
         }
      }
      ...
      override protected function finalParcelAdjustment(param1:ContainerController) : void
      {
         ...
         while(_loc7_ < this._curLineIndex)
         {
            ...
            _loc9_ = _loc8_.getTextLine();
            if(_loc9_.hasGraphicElement)
            {
               _loc12_ = _textFlow.findLeaf(_loc8_.absoluteStart);
               _loc13_ = _loc8_.tlf_internal::getLineTypographicAscent(_loc12_,_loc12_.getAbsoluteStart());
               if(!_loc6_)
               {
                  _loc3_ = Math.min(_loc8_.y + _loc8_.ascent - _loc13_,_loc3_);
               }
               else
               {
                  _loc4_ = Math.max(_loc8_.x + _loc13_,_loc4_);
               }
            }
            _loc11_ = _loc8_.paragraph.computedFormat;
            if(_loc11_.direction == Direction.LTR)
            {
               _loc10_ = Math.max(_loc8_.lineOffset,0);
            }
            else
            {
               _loc10_ = _loc11_.paragraphEndIndent;
            }
            if(_loc6_)
            {
               _loc3_ = Math.min(_loc8_.y - _loc10_,_loc3_);
            }
            else
            {
               _loc2_ = Math.min(_loc8_.x - _loc10_,_loc2_);
            }
            _loc7_++;
         }
         if(_blockProgression == BlockProgression.RL)
         {
            _loc2_ -= _lastLineDescent;
         }
      ...
      override protected function composeNextLine() : TextFlowLine
      {
         ...
         var _loc2_:Boolean = Boolean(_loc1_) && (!_loc1_.tlf_internal::isDamaged() || _loc1_.validity == FlowDamageType.GEOMETRY);
         ...
         {
            _loc3_.tlf_internal::clearDamage();
         }
      ...
      protected function createTextLine(param1:TextFlowLine, param2:int, param3:Number, param4:Number, param5:Number) : TextFlowLine
      {
         ...
         if(param1 == null)
         {
            _loc6_ += Number(_curParaFormat.textIndent);
         }
         var _loc7_:Number = param5;
         param5 -= Number(_curParaFormat.paragraphEndIndent) + _loc6_;
         if(param5 < 0)
         {
            param5 = 0;
         }
         ...
         var _loc9_:TextBlock = _curParaElement.tlf_internal::getTextBlock();
         if(_loc8_)
         {
         ...
         var _loc11_:uint = uint(_loc10_.location);
         if(_loc11_ & TextFlowLineLocation.FIRST)
         {
            _loc10_.tlf_internal::setSpaceBefore(Number(_curParaFormat.paragraphSpaceBefore));
         }
         if(_loc11_ & TextFlowLineLocation.LAST)
         {
            _loc10_.tlf_internal::setSpaceAfter(Number(_curParaFormat.paragraphSpaceAfter));
         }
         return _loc10_;
      }
   }
